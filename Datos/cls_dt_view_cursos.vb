...
Public Class cls_dt_view_cursos
    ...
    Public Function List_Cursos_Periodo(ByVal id_periodo As Integer, ByVal id_plan_est As Integer) As List(Of cls_n_curso_reporte)
        Dim lista_cursos As New List(Of cls_n_curso_reporte)
        Dim DataReader As MySqlDataReader
        Dim obj_cursos As cls_n_curso_reporte
        conn = objConexion.Open
        ...
        comando.CommandText = "SELECT v.periodo, v.curso, v.grupo, v.profesor, v.horario, v.aula, v.matriculados FROM view_curso_periodo v, curso_plan_est c where c.id_curso = v.id_curso and v.id_periodo =" + id_periodo.ToString + " and c.id_plan_est =" + id_plan_est.ToString + " and v.matriculados >=1 order by v.horario,v.curso asc"
        DataReader = comando.ExecuteReader
        While DataReader.Read
            obj_cursos = New cls_n_curso_reporte
            obj_cursos.Reporte_Curso_Periodo = DataReader(0)
    ...
    'SELECT v.periodo, v.curso, v.grupo, v.profesor, v.horario, v.aula, v.matriculados FROM view_curso_periodo v, curso_plan_est c where c.id_curso = v.id_curso and v.id_periodo ="9" and c.id_plan_est ="1";
End Class
